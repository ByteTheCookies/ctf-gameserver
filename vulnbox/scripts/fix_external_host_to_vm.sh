...
if [[ "$MODE" == "--check" ]]; then
  ...
  iptables -t nat -S POSTROUTING | grep -E "(${HOST_SUBNET}|${VM_SUBNET}|${BR_IF})" || true
  exit 0
fi
...
# SNAT host->VM traffic to the bridge gateway source. This avoids asymmetric return-path issues
# when VM-side policy/routing drops direct replies to 10.81.0.0/16.
