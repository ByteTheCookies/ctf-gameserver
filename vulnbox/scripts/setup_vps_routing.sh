...
delete_conflicting_raw_drop_rules() {
  local bridge_if="$1"
  local line
  mapfile -t line < <(iptables -t raw -S PREROUTING | grep -E -- "-A PREROUTING -d 10\\.(30|60)\\.[0-9]+\\.1/32? ! -i ${bridge_if} -j DROP|-A PREROUTING -d 10\\.(30|60)\\.[0-9]+\\.1 ! -i ${bridge_if} -j DROP" || true)
  for l in "${line[@]}"; do
...
if [[ "$MODE" == "--check" ]]; then
  ...
  ip -4 route show "$SUPPORT_VM_SUBNET" || true
  ip -4 route get 10.60.1.1 from 10.81.1.1 iif "$VPN_IF" || true
  ip -4 route get 10.30.1.1 from 10.81.1.1 iif "$VPN_IF" || true
  iptables -S FORWARD | grep -E "(${VPN_IF}|${BR_IF}|10\\.30\\.|10\\.60\\.|10\\.81\\.)" || true
  iptables -S DOCKER-USER | grep -E "(${VPN_IF}|${BR_IF}|10\\.30\\.|10\\.60\\.|10\\.81\\.)" || true
  iptables -t raw -S PREROUTING | grep -E "(10\\.30\\.|10\\.60\\.|${VPN_IF}|${BR_IF})" || true
  iptables -t nat -S POSTROUTING | grep -E "(10\\.30\\.|10\\.60\\.|10\\.81\\.|${VPN_IF}|${BR_IF})" || true
  exit 0
fi
...
ip route replace "$SUPPORT_VM_SUBNET" dev "$BR_IF"

echo "[+] Removing conflicting raw/PREROUTING drops for 10.30.x.1 and 10.60.x.1"
...
ensure_rule raw PREROUTING 1 -i "$VPN_IF" -d "$SUPPORT_VM_SUBNET" -j ACCEPT
...
ensure_rule filter FORWARD 1 -i "$VPN_IF" -o "$BR_IF" -s "$HOST_SUBNET" -d "$SUPPORT_VM_SUBNET" -j ACCEPT
ensure_rule filter FORWARD 1 -i "$BR_IF" -o "$VPN_IF" -s "$SUPPORT_VM_SUBNET" -d "$HOST_SUBNET" -j ACCEPT
ensure_rule filter DOCKER-USER 1 -i "$VPN_IF" -o "$BR_IF" -d "$SUPPORT_VM_SUBNET" -j ACCEPT
ensure_rule filter DOCKER-USER 1 -i "$BR_IF" -o "$VPN_IF" -s "$SUPPORT_VM_SUBNET" -j ACCEPT
...
ensure_rule nat POSTROUTING 1 -s "$HOST_SUBNET" -d "$SUPPORT_VM_SUBNET" -o "$BR_IF" -j MASQUERADE
ensure_rule nat POSTROUTING 1 -s "$SUPPORT_VM_SUBNET" -o "$VPN_IF" -j MASQUERADE
